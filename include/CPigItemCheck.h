...
public:
    ...
    CPigItemCheck(int idComponente,int px, int py, int alt,int larg,int largCheckBox, char *nomeArq,char *labelItem,int retiraFundo=1,int janela=0):
        CPigComponente(idComponente,px,py,alt,larg,nomeArq,janela){
        timer = NULL;
        largFrame = largOriginal/6;
        marcado = false;
        posLabel = COMPONENTE_DIREITA;
        largCheck = largCheckBox;
        SetLabel(labelItem);
        DefineEstado(COMPONENTE_NORMAL);
    }
